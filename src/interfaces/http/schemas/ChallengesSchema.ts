...
export class ChallengesSchema extends BaseValidateClass<Create, Update, UpdateStatus> {
  joi;
  createError;
  ...
  validateCreate(dataToValidate: Create): IValidade {
    const schema = this.joi.object({
      cha_wer_id: this.joi.number().required(),
  ...
  validateUpdateStatus(dataToValidate: UpdateStatus): IValidade {
    const schema = this.joi.object({
      cha_status: this.joi.number().valid(0, 1).required(),
    });
